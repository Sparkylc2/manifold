...
BarnesHutGravityForceGenerator::BarnesHutGravityForceGenerator(
    double G, double theta, double softening, double min_cell_size)
    : m_G(G), m_theta(theta), m_softening(softening),
      m_min_cell_size(min_cell_size) {}

void BarnesHutGravityForceGenerator::apply(SystemState *state) {
...
BarnesHutGravityForceGenerator::Node *
BarnesHutGravityForceGenerator::alloc_node() {
...
BarnesHutGravityForceGenerator::Node *
BarnesHutGravityForceGenerator::build_tree(SystemState *state) {
    if (!state || state->num_b <= 0) {
        return nullptr;
    }
...
void BarnesHutGravityForceGenerator::insert_body(Node *node, int body_index,
                                                 SystemState *state) {
    if (node->is_leaf) {
...
void BarnesHutGravityForceGenerator::subdivide(Node *node) {
...
void BarnesHutGravityForceGenerator::compute_mass(Node *node,
                                                  SystemState *state) {
    node->total_mass = 0.0;
...
void BarnesHutGravityForceGenerator::accumulate_force(int body_index,
                                                      Node *node,
                                                      SystemState *state,
                                                      Vector2d &out_force) {
    ...
    for (int i = 0; i < 4; ++i) {
        if (node->children[i]) {
            accumulate_force(body_index, node->children[i], state, out_force);
        }
    }
}
int BarnesHutGravityForceGenerator::child_index(const Node *node,
...
Vector2d BarnesHutGravityForceGenerator::child_centre(const Node *node,
                                                      int quadrant) const {
