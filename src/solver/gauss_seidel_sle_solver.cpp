...
bool GaussSeidelSLESolver::solve(SparseMatrix<double> &J, VectorXd &W,
...
bool GaussSeidelSLESolver::solve_with_limits(SparseMatrix<double> &J,
                                             VectorXd &W, VectorXd &right,
                                             VectorXd &limits, VectorXd *result,
                                             VectorXd *previous) {
    return solve_impl(J, W, right, &limits, result, previous);
}

bool GaussSeidelSLESolver::solve_impl(SparseMatrix<double> &J, VectorXd &W,
    ...
    for (int iter = 0; iter < m_max_iterations; ++iter) {
        ...
        for (int i = 0; i < m; ++i) {
            ...
            double new_lambda = old_lambda + (right[i] - a_dot_x) / m_diag[i];
...
void GaussSeidelSLESolver::precompute_rows(SparseMatrix<double> &J, VectorXd &W,
                                           int m, int n) {
    m_rows.resize(m);
    m_diag.resize(m);
