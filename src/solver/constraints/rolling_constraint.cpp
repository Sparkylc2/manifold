#include <manifold/solver/constraints/rolling_constraint.h>
...
void RollingConstraint::calculate(Output *output, SystemState *state) {
    ...
    // body 1 (roller): ds/dp1 = d, ds/dtheta1 = 0, dC0/dtheta1 = -1
    ...
    // J_dot
    const double d_dx_dq3_dot = -cos_q3 * q3_dot * mx + sin_q3 * q3_dot * my;
    const double d_dy_dq3_dot = -sin_q3 * q3_dot * mx - cos_q3 * q3_dot * my;
    ...
    output->J_dot[1][2] = (d_dy_dq3_dot * delta_x + d_dy_dq3 * delta_x_dot) +
...
void RollingConstraint::set_base_body(RigidBody *body) { m_bodies[0] = body; }
void RollingConstraint::set_rolling_body(RigidBody *body) {
    m_bodies[1] = body;
}

void RollingConstraint::set_local_origin(const Vector2d &origin) {
    m_local = origin;
}
void RollingConstraint::set_direction(const Vector2d &dir) {
    m_direction = dir;
}
void RollingConstraint::set_radius(double r) { m_radius = r; }

void RollingConstraint::set_ks(double ks) { m_ks = ks; }
void RollingConstraint::set_kd(double kd) { m_kd = kd; }
