...
DistanceConstraint::DistanceConstraint() : Constraint(1, 2) {
    m_distance = 1.0;
    m_ks = 10.0;
    m_kd = 1.0;
}

void DistanceConstraint::calculate(Output *output, SystemState *state) {
...
void DistanceConstraint::set_bodies(RigidBody *a, RigidBody *b) {
    m_bodies[0] = a;
    m_bodies[1] = b;
}

void DistanceConstraint::set_distance(double d) { m_distance = d; }

// automatically compute from current body positions
void DistanceConstraint::set_distance_from_bodies() {
    if (m_bodies[0] && m_bodies[1])
        m_distance = (m_bodies[0]->p - m_bodies[1]->p).norm();
}

double DistanceConstraint::distance() const { return m_distance; }

void DistanceConstraint::set_ks(double ks) { m_ks = ks; }
void DistanceConstraint::set_kd(double kd) { m_kd = kd; }
