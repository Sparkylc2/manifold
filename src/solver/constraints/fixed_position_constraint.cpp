...
void FixedPositionConstraint::calculate(Output *output, SystemState *state) {
...
void FixedPositionConstraint::set_body(RigidBody *body) { m_bodies[0] = body; }

void FixedPositionConstraint::set_world_position(const Vector2d &w) {
    m_world = w;
}
void FixedPositionConstraint::set_local_position(const Vector2d &l) {
    m_local = l;
}

void FixedPositionConstraint::set_ks(double ks) { m_ks = ks; }
void FixedPositionConstraint::set_kd(double kd) { m_kd = kd; }

const Vector2d &FixedPositionConstraint::world_position() const {
    return m_world;
}
const Vector2d &FixedPositionConstraint::local_position() const {
    return m_local;
}
