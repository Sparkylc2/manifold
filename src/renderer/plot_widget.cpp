...
PlotWidget::PlotWidget(const char *label, Color line_color, int max_history,
                       Transform transform)
    : m_label(label), m_line_color(line_color), m_max_history(max_history),
      m_transform(transform ? transform : [](double v) { return v; }) {}

void PlotWidget::configure(const char *label, Color line_color, int max_history,
                           Transform transform) {
    m_label = label;
...
void PlotWidget::push(double value) {
    m_data.push_back(value);
    while ((int)m_data.size() > m_max_history)
        m_data.pop_front();
}

void PlotWidget::clear() { m_data.clear(); }

void PlotWidget::render(Renderer *r, int bx, int by, int bw, int bh) const {
    ...
    if (vmin < 0 && vmax > 0) {
        ...
        r->draw_screen_line(bx, zy, bx + plot_bw, zy, 1.0f, theme.grid_axis);
    }
...
void PhasePlot::configure(const char *xlabel, const char *ylabel, Color color,
                          int max_history) {
    m_xlabel = xlabel;
    m_ylabel = ylabel;
    m_color = color;
...
void PhasePlot::push(double x, double y) {
    m_xs.push_back(x);
    m_ys.push_back(y);
...
void PhasePlot::clear() {
    m_xs.clear();
    m_ys.clear();
}

void PhasePlot::render(Renderer *r, int bx, int by, int bw, int bh) const {
