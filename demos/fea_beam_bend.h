...
class FeaBeamBend : public DemoBase {
  public:
    ...
    void initialize() override {
        ...
        build_beam();
        const Vector2d spring_u_attach = Vector2d(ROWS * 1e-6 * 2 / 3.0, 0);
        const Vector2d spring_l_attach = Vector2d(ROWS * 1e-6 / 3.0, 0);

        m_spring_u =
            Coupling::FeaSpringForce(spring_u_attach, m_body.get(), 50, K, C);
        m_spring_l =
            Coupling::FeaSpringForce(spring_l_attach, m_body.get(), 100, K, C);

        m_cyl_c = Vector2d(m_base.x() - 1.1, 0.85 * BEAM_H);
  ...
  private:
    ...
    void build_beam() {
        m_base.x() = 0.30 * COLS * CELL;
        m_base.y() = 0.50 * COLS * CELL;

        FEA::Mesh mesh;
        for (int j = 0; j <= NH; j++)
            for (int i = 0; i <= NW; i++)
                mesh.add_node(
                    Vector2d(m_base.x() - 0.5 * BEAM_W + BEAM_W * i / NW,
                             m_base.y() - 0.5 * BEAM_H + BEAM_H * j / NH));
    ...
    Coupling::FeaSpringForce m_spring_u, m_spring_l;
    ...
    Vector2d m_base = {0.0, BEAM_H / 2.0};
