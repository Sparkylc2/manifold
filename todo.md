
ideas:
- turbojet modelled fully including temperature and vorticity in the thing etc etc. maybe use a reduced order model
- really cool 3D pde in time for visuals
- get the thermal stuff down so we can assign thermal solving to bodies, and have it couple with everything
- have a "speaker" and "microphone" in a fluid field which supports compressibility
- show strings or a drum vibrating after being struck
- a controlled robot arm with multiple dof balancing a ball on a plate
x a pendulum kick-up demo
x simulating circuits
- see if we could technically build a clock
x add beam bending or some 2d finite element method
x add self landing rocket
x show an integrator circuit with op-amps
- simulate a motor from scratch (ie the electrical physics behind how a motor works)
- see if we can maybe create an "autopilot" guidance system, simulate a rocket launch with a full state model and do guidance using an atmosphere and space model
x create a simple 2D FEA solver with coupling
- simulate convective and conductive heat transfer through an fea to simulate stuff such as a heater, or radiator
